...
if len(sys.argv) > 1 and sys.argv[1] == "-cli":
    ...
    exit1.place(x=435,y=0)
    #for change username
    change_username1=customtkinter.CTkButton(master=root,text="change username",corner_radius=10,command=change_username)
    ...
    statusConnect.set("Connect")
    clientType = 1
    '''a=customtkinter.CTkRadioButton(master=root, text="Client", variable=clientType,value=0, command=toOne)
    a.pack(anchor=E)
    b=customtkinter.CTkRadioButton(master=root, text="Server", variable=clientType,value=1, command=toTwo)
    b.pack(anchor=E)'''
    connecter = customtkinter.CTkButton(master=root, textvariable=statusConnect,
                       command=lambda: connects(clientType))
    connecter.pack(padx=0,pady=0)
