...
def connect_on_port(master):
    ...
    location.focus_set()
    Label(top, text="Port Adr:").grid(row=1)
    port = Entry(top)
...
if len(sys.argv) > 1 and sys.argv[1] == "-cli":
    ...
    file_menu.add_command(label="Save chat", command=lambda: savechat())
    file_menu.add_command(label="Change username",
                          command=lambda: change_username(root))
    file_menu.add_command(label="Exit", command=lambda: root.destroy())
    menubar.add_cascade(label="File", menu=file_menu)
    #for connect menu
    connection_menu = Menu(menubar, tearoff=0)
    connection_menu.add_command(label="Quick Connect", command=Quickconnect)
    connection_menu.add_command(
        label="Connect on port", command=lambda: connect_on_port(root))
    connection_menu.add_command(
