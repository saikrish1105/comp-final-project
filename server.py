...
if len(sys.argv) > 1 and sys.argv[1] == "-cli":
    ...
    #for exit
    exit1=customtkinter.CTkButton(master=root,text="Exit",corner_radius=10,height=29,width=110,command=root.destroy)
    exit1.pack(padx=20, pady=10)
    exit1.place(x=580,y=0)
    #for getiing old chat
    chat=customtkinter.CTkButton(master=root,text="Prev. Chat",corner_radius=10,height=29,width=110,command=revchat)
    ...
    main_body_text.insert(END, f"Welcome to the chat program! \nDate: {date_time[0:10]} Starttime: {date_time[11:19]} \nType-'/' to \nConnect to a port or \nChange your nickname!")
    main_body_text.config(state=DISABLED)
